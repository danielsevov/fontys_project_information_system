...
public interface DAO<E extends Serializable, K extends Serializable> extends
     ...
     *
     * @return the token
     * @throws Exception on error, eg database connection failure
     */
    ...
    default List<String> generatedFields() {
        ...
        return Arrays.stream(declaredFields).filter(DAO::isGenerated).map(Field::getName).collect(toList());
    }
     ...
     * @return the updated entity
     */
    E updateColumn(E e, String column, Object value);
}
